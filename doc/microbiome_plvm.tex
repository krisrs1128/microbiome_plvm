...
means for the $\beta_{vk}$, for each word v$, and the $y$-axis gives the
associated standard deviation, along $K = 1$, for each $v$.
...
samples $D \in \{20, 100\}$ but fix the number of latent factors $K$ at 2. Figure
\ref{fig:zinf_thetas} summarizes the estimation error across regimes.
...
\begin{figure}[!p]
  ...
  \caption{A summary of the errors and uncertainty across models and regimes for
    terms in Figure \ref{fig:lda_contours}, after transforming to a
    log-scale. \label{fig:beta_errors_lda} }
\end{figure}
