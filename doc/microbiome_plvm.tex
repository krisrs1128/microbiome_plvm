...
We review a few of the statistical modeling techniques that are applied
throughout this work. Many of these techniques have been borrowed from text
analysis, thinking of the bacterial counts matrix as a biological analog of the
...
explored in relatively limited settings \citep{shafiei2015biomico,
...
modeling applicable to count matrices \citep{blei2003latent}. For now, we adopt
the usual text-analysis terminology, where the count matrices of interest are
counts of terms across documents, though in section
\ref{sec:microbiome_vs_text_analysis} we clarify the connection to the language
of microbiome analysis. Suppose there are $D$ documents across $V$ terms, and
that these documents are assumed to be mixtures of $K$ underlying topics, where
a topic is defined as a distribution over words.
...
\simplex^{V}$. Suppose there are $N_{d}$ words in the $d^{th}$ document. Then,
the generative mechanism for the observed data set is
\begin{align*}
w_{dn} \vert \left(\beta_{\cdot k}\right)_{k = 1}^{K}, z_{dn} &\overset{iid}{\sim} \Mult\left(1, \beta_{\cdot z_{dn}}\right) \text{ for } d = 1, \dots, D  \text{ and } n = 1, \dots,  N_{d} \\
...
\beta_{\cdot k} &\overset{iid}{\sim} \Dir\left(\gamma\right) \text{ for } k = 1, \dots, K.
\end{align*}
...
column-wise, $B = \left(\beta_{1}, \dots, \beta_{K}\right)$. Geometrically, LDA
identifies samples with points in the convex hull of $K$ topics
$\left(\beta_{k}\right)$ on the $V$-dimensional simplex.
...
$V$-dimensional simplex. This reasoning is the starting point of the Dynamic
Unigram model \citep{blei2006dynamic}. This curve reflects the gradual evolution
...
model
\begin{align*}
x_{d\cdot} \vert \mu_{t\left(d\right)}  &\overset{iid}{\sim} \Mult\left(\sum_{v} x_{dv}, S\left(\mu_{t\left(d\right)}\right)\right) \text{ for } d = 1, \dots, D\\
...
distributions. In our experiments, we place an inverse-gammma prior on
...
$\theta_{d} \in \mathcal{S}^{K - 1}$ and $\beta_{k} \in \mathcal{S}^{V - 1}$.
...
distribution as a Gamma mixture of Poissons, this is a natural model to
considering when facing overdispersed counts, which arise frequently in genomic
and microbiome settings \citep{love2014moderated, mcmurdie2014waste}. In
practice, the hyperparameters $a_{0}, b_{0}, c_{0}$, and $d_{0}$ are unknown,
and in all experiments, we optimize over them according to the procedure in
\citep{carpenter2016stan}, though it would also be possible to add a level to
the hierarchy and place vague nonnegative priors on these parameters.

In our simulation studies, we will consider a slight variant of this model, in
similar to the proposal of \citep{pierson2015zifa}, which independently sends
entries of $X$ to zero with probability $p_{0}$. In our experiments, we assume this
zero-inflation probability is known. This procedure is denoted by Z-NMF.
...
fields are often superficial. For example, we can map between the most common
field-specific terms as follows,
...
\begin{itemize}
  ...
  \item \textbf{Term} $\iff$ \textbf{Bacteria}: The fundamental features
    ...
    bacteria (microbiome analysis). More formally, by bacteria, we mean
    Ribosomal Sequence Variants \citep{callahan2016dada2}, though we call these
    bacteria for simplicity
...
contrast, even large microbiome data sets, like the one studied by
\cite{human2012structure}, typically only have on the order of thousands of
...
simulated data set, we perform estimation using VB, MCMC sampling, and a parametric bootstrap.
In more detail, this bootstrap procedure
fits VB to the original data, simulates $B = 1000$ new data
...
etc. Judging from the aligned densities in Figures \ref{fig:beta_contours_v10},
\ref{fig:beta_contours_v50}, and \ref{fig:beta_contours_nmf}, this ad-hoc
alignment procedure seems sufficient.

Figure \ref{fig:beta_contours_v10} displays the true and posterior $\beta_{k}$ for
the experiments with $V = 10$ features, while varying other simulation
...
number of samples $D$. The analogous figure when $V = 50$ is provided in the
...
Interestingly, in the $V = 50$ case, VB appears more accurate than either the
...
parameter from their true values, after alignment, and (2) the concentration
of these posteriors around their means? Figure \ref{fig:beta_errors_lda} addresses these questions
directly -- the $x$-axis gives the RMSE of the square-root-transformed posterior
means for the $\beta_{vk}$, for each $v$, and the $y$-axis gives the
associated standard deviation, along $K = 1$, for each $v$. As in Figure
\ref{fig:beta_contours_v10}, the first row of column numbers gives the total count
$N$ in each sample, and the second gives $D$, the number of samples. The grey
line within each panel is the identity line, where the error equals one standard
deviation.
...
before is visible in the ordering across colors in this figure. Note that few
points lie above the identity standard deviation line -- this suggests that the
posteriors may be misleadingly narrow. Finally, we note that,
the scale of errors and concentration in the $V = 10$ and $V = 50$
cases are comparable. Considering that there is the same number of documents
available for estimating each $\beta_{vk}$, this is not entirely surprising.
...
the expected total count per sample $\Earg{N}$, rather than $N$. We perform
inference both when $p_{0} > 0$ is incorporated into the model (in this case, we
actually provide the true $p_{0}$ for inference), and when $p_{0}$ is
(incorrectly) assumed to be zero. To modulate $\Earg{N}$, we vary $a_{0}$ and $b_{0}$ so
that $\Earg{N} = \frac{b_{0}}{a_{0}}V \in \{20, 50, 100\}$, matching the LDA experiment.
As before, we vary the number of samples $D \in \{20, 100\}$ but fix the number
of latent factors $K$ at 2. Figure \ref{fig:beta_errors_zinf} summarizes the
estimation error across regimes, and is read in the same way as Figure
\ref{fig:beta_errors_lda}, except that
\begin{enumerate}
...
to distinguish between models that treat the mean
$\left(c\Theta\right)\left(\frac{1}{c} B\right)^{T}$ instead of $\Theta B^{T}$
is through the prior, and the prior may not be strong enough to guide inference
to the right scaling.

Further, examining Supplementary Figure \ref{fig:beta_contours_nmf}, we note that
the different inferential procedures produce quite different contours, even when
$\Earg{N}$ is large. There also seem to be cases where MCMC sampling fails to
concentrate, and cases for VB and the bootstrap that concentrate around the
wrong value, though this may be related to rescaling. Generally, we do not find
a direct probabilistic-programming implementation of the GaP or Z-GaP models
reliable, even when $p_{0}$ is known.
...
less meaningful, since there are only 56
...
between days 12-23 and 41-51. Topic 1 becomes at the start of the first time
course, suggesting that it reflects typical bacterial community structure. Topic
2 seems to represent those bacteria that were present initially but are
eliminated during the first time course, though there is a hint of a recovery at
the end of sampling. This topic seems most closely related to the finding
reported in \citep{dethlefsen2011incomplete} that Subject F experienced
long-term antibiotic effects on bacterial community composition. Topics 2 and 4
both seem to be overrepresented during the antibiotic treatments. Topic 2 is
elevated immediately after the antibiotic treatment; in contrast, topic 3 seems
to become elevated more gradually. Further, topic 4 appears to be more common in
...
estimated topics. Within a row, we show credible intervals associated with the
posterior samples for each bacterium. Different colors identify different
...
diversity during antibiotic time courses. We also note ``bumps'' of neighboring
bacteria with similarly elevated topic probabilities. It is encouraging that,
even without specifying smoothness along the phylogenetic tree in the prior for
the $\beta_{k}$s, such smoothness emerges in the fitted model.

One disadvantage of plotting the posteriors for the $\beta_{k}$ in this way is
that it is difficult to determine the identify of any bacterial species
associated with any specific credible interval. To address this, we have
designed an interactive version, available online at
...
\ref{sec:antibiotics_unigram}. Each subpanel corresponds to a single RSV. The
black lines represent observed time series. Note that the $y$-axis scales vary,
as some RSVs are much more abundant than others. Each dot is a simulated
...
in RSV 343 is not captured in any posterior predictive samples. Similar
oversmoothing is visible in RSVs 1036, 1116, and 1463. A more startling type of
...
highly abundant RSVs disappear after the second time course, and hence they are
not captured by the global LDA summary. This suggests a technique for
highlighting outlier RSVs: we can look at the average discrepancy between
observed series and their posterior predictive samples.

On the other hand, for the Dynamic Unigram model, the posterior predictive distribution places most
...
seems over-parameterized, simply memorizing the input.
...
Dynamic Unigram model, respectively. For LDA, the posterior predictive samples have
comparable top four eigenvalues, but rapidly drop-off between the fourth and
fifth eigenvalues. On the other hand, the observed data have a more steady
...
different models and inference mechanisms depending on the overall data generation regime. On a real
microbiome data analysis problem, we have characterized the advantages and
limitations of two probabilistic modeling techniques. Rather than focusing
...
them -- in these richly structured scenarios.
...
\begin{figure}[!p]
  \centering\includegraphics[scale=0.2]{figure/beta_contours_v10}
  \caption{Different inference algorithms for LDA produce different uncertainty
    ...
    posterior samples from the inference procedure given as row labels. The
...
\begin{figure}[!p]
  \centering
  \includegraphics{figure/beta_errors_lda}
...
\begin{figure}[!p]
  \centering
  \includegraphics{figure/beta_errors_nmf}
...
\begin{figure}[!p]
  ...
  \caption{Each credible interval describes an approximate posterior for one
    ...
    relatedness, and the $y$-axis give transformed values of the RSV probability
    under that topic. In particular, before transforming, the sum of points
    across a single row would always be 1. Note the low diversity within Topic
    4.} \label{fig:antibiotics_lda_beta}
\end{figure}

\begin{figure}[!p]
  \centering
  \includegraphics[scale=1]{figure/antibiotics_unigram_mu-F}
  \caption{Each posterior credible interval refers to one $\mu_{vt}$. The rows
    are a subset of times $t$ around the first antibiotic time course.
    Otherwise, this display is read in the same way as Figure
    \ref{fig:antibiotics_lda_beta}. This view provides one way of smoothing
    abundance time series, to see how different species respond to antibiotic
    treatment. \label{fig:antibiotics_unigram_theta} }
\end{figure}

\begin{figure}[!p]
  \centering
  \includegraphics[scale=0.2]{figure/posterior_check_evals-F}
...
\begin{figure}[!p]
  \centering
  \includegraphics[scale=0.2]{figure/posterior_check_ts-F}
  \caption{We can visualize the simulated time series for a subset of RSVs
    and compare them with the observed ones, as a posterior check. Each panel
    represents one RSV. The black lines represent the observed
    $\asinh$-transformed abundances for subject F over time. The blue and purple
    dots give the posterior predictive realizations for these RSVs over time,
...
\begin{figure}[!p]
  \centering
  \includegraphics[scale=0.2]{figure/beta_contours_v50}
...
\begin{figure}[!p]
  ...
  \caption{As a posterior check, we compare the observed with simulated
    data quantiles, using a qq-plot. To reduce overlap, we have introduced a a
...
\begin{figure}[!p]
  \centering
  \includegraphics[scale=0.15]{figure/posterior_check_loadings-F}
  \includegraphics[scale=0.15]{figure/posterior_check_scores-F}
  \caption{The eigenvalues displayed in Figure
    ...
    pair of panels give loadings for each timepoint, while the right pair
    provide scores for each RSV. The individual posterior samples have been
...
\begin{figure}[!p]
  \centering
  \includegraphics{figure/beta_contours_nmf}
