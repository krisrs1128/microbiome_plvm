...
provided in the Supplementary Figure \ref{fig:lda_v50}.
...
samples seem somewhere between the VB and MCMC sampled distributions in terms of
variability within each feature. As the number of samples or total count within
samples increases, the three methods become indistinguishable. Interestingly,
when $D$ and $N$ are small ($N = D = 20$), VB appears more accurate than either
the MCMC or bootstrapping approaches, whose posterior means all lie along the
diagonal, corresponding to the failure to estimate distinct topics, and instead
defaulting to marginal feature counts across samples. This effect is especially
pronounced in the case $V = 50$.
...
features of interest are (1) how close are the posterior means for each
parameter to their true values, after alignment, and (2) how concentrated are
the posteriors? Figure \ref{fig:beta_errors_lda} addresses these questions
...
line within each panel is the identity line, where the error equals one SD.
