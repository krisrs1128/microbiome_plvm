...
\ref{sec:microbiome_vs_text_analysis} we clarify the connection to 
microbiome analysis. Suppose there are $D$ documents across $V$ terms, and
...
Let $\theta_{d} \in \simplex^{K - 1}$ represent the $d^{th}$ topic's mixture over the
...
column-wise, $B = \begin{pmatrix}beta_{1}, \dots, \beta_{K}\end{pmatrix}$. Geometrically, LDA
...
distributions. In our experiments, we place a vague inverse-gammma prior on
...
\citep{kucukelbir2015automatic, carpenter2016stan, canny2004gap} which proposes
...
\begin{itemize}
  ...
  \item \textbf{Term} $\iff$ \textbf{Bacteria}: The fundamental features
    ...
    bacteria for simplicity of exposition.
  \item \textbf{Topic} $\iff$ \textbf{Community}: For interpretation, it is
    ...
    ``communities'' -- one common breakdown is healthy vs. diseased. Different
...
\begin{figure}[!p]
  \centering
  \includegraphics[scale=0.15]{figure/beta_contours_nmf}
