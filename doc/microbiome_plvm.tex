...
describe the data of interest. Here, we describe model asssessment via posterior
predictive checks \citep{rubin1984bayesianly, gelman2013pilosophy}. In this
...
marginal $p\left(x^{\ast}\right) = \int p\left(x^{\ast} \vert \theta\right)
\hat{p}\left(\theta \vert x \right)d\theta$, where $x$ is the original data and
$\hat{p}\left(\theta \vert x\right)$ is an estimate computed from posterior
samples. For each of these simulated data sets, the characterizing statistics
$T_{k}\left(x_{s}\right)$ are computed. Graphically comparing the true data
$T_{k}\left(x\right)$ with the histogram of model-fit simulated
$T_{k}\left(x^{\ast}\right)$ suggests ways in which the posited model fits
(observed $T_{k}\left(x\right)$ lying in the bulk of the
$T_{k}\left(x^{\ast}_{s}\right)$) or fails to fit ($T_{k}\left(x\right)$ far
from the bulk of $T_{k}\left(x^{\ast}_{s}\right)$) the data well.
...
\begin{figure}[!p]
  ...
  \caption{Each boxplot refers to one $\mu_{vt}$. The rows are a subset of times $t$ around the first antibiotic time course. This view provides one way of smoothing abundance time series, to see how different species respond to antibiotic treatment. \label{fig:antibiotics_unigram_theta} }
\end{figure}
